...
def blitc(pin_list #list must have pins in the order: a-g, df
    ...
    if str_character or str_character== '1':
...
def sentence(pinlist #list must have pins in the order: a-g, dp
    ...
    for x in string:
        try:
            blitc(pinlist,x)
            time.sleep(time)
            GPIO.output(pin_list,[0,0,0,0,0,0,0,0])

        except:
            error=(x, "is an invalid character and cannot be displayed with a 7 segment unit.")
            raise ValueError(error)


def customchr(pinlist #list must have pins in the order: a-g, dp
